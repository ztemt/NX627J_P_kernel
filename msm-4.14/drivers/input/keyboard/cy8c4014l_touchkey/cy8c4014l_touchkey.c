			...
			struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(buf[0] == '0'){
        new_mode = CYPRESS4000_SLEEP_MODE;
    }else if(buf[0] == '1'){
        new_mode = CYPRESS4000_WAKEUP_MODE;
    }else{
        dev_err(dev, "mode set failed, 0: sleep, 1: wakeup\n");
        return count;
    }

    mode = i2c_smbus_read_byte_data(client, CYPRESS4000_TOUCHKEY_MODE);
    if(mode == new_mode){
        dev_err(dev, "mode already is %s now!!\n", (new_mode == CYPRESS4000_SLEEP_MODE)?"sleep":"wake");
        return count;
    }

    ret = i2c_smbus_write_byte_data(client,CYPRESS4000_TOUCHKEY_MODE, new_mode);
    if(ret<0){
        dev_err(&client->dev, "Set %s mode error!\n", (new_mode == CYPRESS4000_SLEEP_MODE)?"sleep":"wake");
        return count;
    }

    dev_err(dev, "set touchkey to %s mode!!\n", (new_mode == CYPRESS4000_SLEEP_MODE)?"sleep":"wake");
    return count;
}
...
#define CYPRESS_AVDD_VOL 2800000
