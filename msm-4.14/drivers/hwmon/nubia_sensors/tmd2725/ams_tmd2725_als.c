...
{
	int ret;
		/*nubia add  tmd2725/tmd3725 code*/
	if(chip->device_revid == TMD2725_REVID)	{
		ret = ams_i2c_blk_read(chip->client, TMD2725_REG_CH0DATA,
			&chip->shadow[TMD2725_REG_CH0DATA], 4 * sizeof(u8));
	}else{
		ret = ams_i2c_blk_read(chip->client, TMD2725_REG_CH0DATA,
			&chip->shadow[TMD2725_REG_CH0DATA], 8 * sizeof(u8));
	}
	...
	if (ret >= 0) {
		...
		chip->als_inf.als_clear= le16_to_cpu(
			*((const __le16 *) &chip->shadow[TMD2725_REG_CH0DATA]));
		chip->als_inf.als_red= le16_to_cpu(
			*((const __le16 *) &chip->shadow[TMD2725_REG_CH1DATA]));
		chip->als_inf.als_green= le16_to_cpu(
			*((const __le16 *) &chip->shadow[TMD2725_REG_CH2DATA]));
		chip->als_inf.als_blue= le16_to_cpu(
			*((const __le16 *) &chip->shadow[TMD2725_REG_CH3DATA]));
		}
		ret = 0;
	}
...
static int tmd2725_get_lux(struct tmd2725_chip *chip)
{
	...
	if (lux < 0) {
		...
		ch0 = (int64_t) chip->als_inf.als_clear;
		ch1 = (int64_t) chip->als_inf.als_red;
		ch2 = (int64_t) chip->als_inf.als_green;
		ch3 = (int64_t) chip->als_inf.als_blue;	
		lux = ch0*C_Coef + ch1*R_Coef + ch2*G_Coef + ch3*B_Coef;      
		lux = lux / D_Factor;
