...
static int tmd2725_check_device_id(struct tmd2725_chip *chip)
{
	...
	if(chip->device_revid == TMD2725_REVID){
	    SENSOR_LOG_INFO("tmd2725 start\n");
    	}else{
	    SENSOR_LOG_INFO("tmd3725 start\n");
	}
...
{
	struct tmd2725_chip *chip = dev_get_drvdata(dev);
	//SENSOR_LOG_ERROR("tmd2725_suspend enter\n");
	AMS_MUTEX_LOCK(&chip->lock);
...
static int tmd2725_resume(struct device *dev)
{
	struct tmd2725_chip *chip = dev_get_drvdata(dev);
	chip->in_suspend = 0;
	//SENSOR_LOG_ERROR("tmd2725_resume enter\n");
	AMS_MUTEX_LOCK(&chip->lock);
	if (chip->als_enabled)
