...
#define DEV_ALS_NAME         "light_back"
...
{
	if (chip->als_cal_data.flag) {
		//SENSOR_LOG_DEBUG_IF(1, "cal_lux:%d\n", chip->als_inf.cal_lux + 1);
		input_report_rel(chip->a_idev, REL_X, chip->als_inf.cal_lux + 1);
	} else {
		//SENSOR_LOG_DEBUG_IF(1, "lux:%d\n", chip->als_inf.prev_lux + 1);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (5 != sscanf(buf, "%10lld,%10lld,%10lld,%10lld,%10lld", &d_factor,&ch0_coef1, &ch1_coef1, &ch0_coef2, &ch1_coef2)){
		return -EINVAL;
}
	AMS_MUTEX_LOCK(&chip->als_lock);
...
struct device_attribute tmd2725_als_attrs[] = {
	__ATTR(enable,        	  0664, tmd2725_als_enable_show,       tmd2725_als_enable_store),
	__ATTR(delay,         	  0664, tmd2725_als_delay_show,        tmd2725_als_delay_store),
	__ATTR(debug,         	  0664, tmd2725_als_debug_show,        tmd2725_als_debug_store),
	...
	__ATTR(calibrate,     	  0664, tmd2725_als_calibrate_show,    tmd2725_als_calibrate_store),
	__ATTR(fac_calibrate, 	  0664, tmd2725_als_fac_calibrate_show,tmd2725_als_fac_calibrate_store),
	__ATTR(als_Itime,     	  0664, tmd2725_als_itime_show,        tmd2725_als_itime_store),
	__ATTR(als_Wtime,     	  0664, tmd2725_als_wtime_show,        tmd2725_als_wtime_store),
	__ATTR(als_gain,          0664, tmd2725_als_gain_show,         tmd2725_als_gain_store),
	__ATTR(als_cpl,           0444, tmd2725_als_cpl_show,          NULL),
	__ATTR(als_thresh_deltaP, 0664, tmd2725_als_deltaP_show,       tmd2725_als_deltaP_store),
	__ATTR(als_auto_gain,     0664, tmd2725_auto_gain_enable_show, tmd2725_auto_gain_enable_store),
	__ATTR(als_lux_table,     0664, tmd2725_lux_table_show,        tmd2725_lux_table_store),
	__ATTR(als_persist,       0664, tmd2725_als_persist_show,      tmd2725_als_persist_store),
	__ATTR(als_ch0,           0444, tmd2725_als_ch0_show,          NULL),
	__ATTR(als_ch1,           0444, tmd2725_als_ch1_show,          NULL),
#ifdef LUX_DBG
	__ATTR(als_adc,           0664, tmd2725_als_adc_show,          tmd2725_als_adc_store),
#endif
};
...
static int tmd2725_als_input_device_init(struct tmd2725_chip *chip)
{
	...
	chip->a_idev = virtual_als_input_dev;
#if 0
