...
#define CONFIG_NUBIA_AOD_MODE_GESTURE_EN
...
/*panel data */
#define GOODIX_TS_BACK_PALM
#define GOODIX_TS_BACK_GESTURE
#define NUBIA_GAME_TOUCH_HANDLE

/*#define NUBIA_BACK_SWITCH_TP*/
#define READY_FRONT_PANEL_MODE        2000

#define READY_BACK_PANEL_MODE          	2001
#define ALREADY_BACK_PANEL_MODE		1001

#define MIRROR_PANEL_OPEN     		1002
#define MIRROR_PANEL_CLOSE     		2003

#define BACK_PANEL_EFFECT_MODE 		2005
#define AOD_BACK_ADJUSTTO_LIGHT


#define BACK_SUSPEND	0
#define BACK_RESUME	1
#define FRONT_SUSPEND 	2
...
struct goodix_ts_core {
	...
	int palm_enabled;
#ifdef GOODIX_TS_BACK_PALM
	int palm_back_enabled;
#endif
	int state_flag;
	/*back panel effect mode */
	int bpe_mode;
	/*when aod and light_effect all exist,next back tp shoudle to resume*/
	int previousEffectState;
	/*tp_state_cmd is back or front to resume or suspend*/
	int tp_in_aod;
	int tp_state_cmd;
...
/* log macro */
#define ts_info(fmt, arg...)	pr_info("[NUBIA_TS back-INF][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define	ts_err(fmt, arg...)		pr_err("[NUBIA_TS back-ERR][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define boot_log(fmt, arg...)	g_info(fmt, ##arg)
#ifdef CONFIG_GOODIX_DEBUG
#define ts_debug(fmt, arg...)	pr_info("[NUBIA_TS back-DBG][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#else
