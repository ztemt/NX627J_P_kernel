 ...
 */
static ssize_t gsx_back_gesture_type_show(struct goodix_ext_module *module,
				char *buf)
{
	int count = 0, i, ret = 0;
	unsigned char *type;
 ...
 */
static ssize_t gsx_back_gesture_type_store(struct goodix_ext_module *module,
...
static ssize_t gsx_back_gesture_enable_show(struct goodix_ext_module *module,
		char *buf)
{
...
static ssize_t gsx_back_gesture_enable_store(struct goodix_ext_module *module,
		const char *buf, size_t count)
{
	unsigned int tmp;
...
static ssize_t gsx_back_gesture_data_show(struct goodix_ext_module *module,
				char *buf)
{
	int count = GSX_KEY_DATA_LEN;
...
const struct goodix_ext_attribute gesture_attrs[] = {
	__EXTMOD_ATTR(type, 0666, gsx_back_gesture_type_show,
		gsx_back_gesture_type_store),
	__EXTMOD_ATTR(enable, 0666, gsx_back_gesture_enable_show,
		gsx_back_gesture_enable_store),
	__EXTMOD_ATTR(data, 0444, gsx_back_gesture_data_show, NULL)
};
int goodix_unregister_gesture(void){
	...
	ts_debug("Start back unregistered gesture module");
	...
	struct goodix_ext_module *module)
{
	...
	ts_debug("back data[0-4]0x%x, 0x%x, 0x%x, 0x%x", temp_data[0], temp_data[1],
	...
	if (QUERYBIT(gsx_gesture->gesture_type, temp_data[2])) {
		/* do resume routine */
		ts_info("back Gesture match success, resume IC");

#ifdef AOD_BACK_ADJUSTTO_LIGHT
	if (aod_light == temp_data[2]) {
		ts_info("now need to resume back panel");
		goodix_resume_state_set(core_data, 1);
		input_report_key(core_data->input_dev, KEY_F15, 1);
		input_sync(core_data->input_dev);
		input_report_key(core_data->input_dev, KEY_F15, 0);
		input_sync(core_data->input_dev);
...
#ifdef GOODIX_TS_BACK_GESTURE
		input_report_key(core_data->input_dev, KEY_F12, 1);
		input_sync(core_data->input_dev);
		input_report_key(core_data->input_dev, KEY_F12, 0);
		input_sync(core_data->input_dev);
#else
...
static int __init goodix_gsx_gesture_init(void)
{
	...
	int result;
	ts_info("back gesture module init");
	...
	gsx_gesture->module.name = "Goodix_back_gsx_gesture";
