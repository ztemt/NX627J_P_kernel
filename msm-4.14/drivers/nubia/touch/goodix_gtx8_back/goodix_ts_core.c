...
#define PINCTRL_STATE_ACTIVE    "pmx_ts_back_active"
#define PINCTRL_STATE_SUSPEND   "pmx_ts_back_suspend"

extern void aod_state_front_tp_to_suspend(void);
extern void front_palm_flag(int *v);
extern int goodix_start_cfg_bin(struct goodix_ts_core *ts_core);
#ifdef GOODIX_TS_BACK_GESTURE
/*This flag is not to resume tp after first back light effect open*/
int firEffectFlag = 1;
/**
...
*/
int panelDeadState = 0;
#endif

/*back_nopanel was used when not connect back panel*/
static int back_nopanel;
		...
		struct device_attribute *attr, char *buf)
{
	...
	int r, cnt = 0;
	if (1 == back_nopanel)
		cnt += snprintf(buf,PAGE_SIZE,"no back panel\n");
	else {
		...
		char *buf)
{
	...
	r = snprintf(buf, PAGE_SIZE, "plam:%d\n",
			ts_dev->palm_state);
	if (r < 0)
		return -EINVAL;
		...
		size_t count)
{
	...
	if(en) {
		ts_info("back_palm open");
	} else {
		ts_info("back_palm close");
	}
	core_data->palm_enabled = en;
	ts_dev->palm_state = en;
	front_palm_flag(&en);
	return count;
}
#endif
#ifdef NUBIA_GAME_TOUCH_HANDLE
		...
		char *buf)
{
	...
	if (BACK_SUSPEND == (core_data->tp_state_cmd))
		r = snprintf(buf, PAGE_SIZE, "back_suspend\n");
	else if (BACK_RESUME == (core_data->tp_state_cmd))
		r = snprintf(buf, PAGE_SIZE, "back_resume\n");
	else if (FRONT_SUSPEND == (core_data->tp_state_cmd))
		r = snprintf(buf, PAGE_SIZE, "front_suspend\n");
		...
		size_t count)
{
	...
	ts_info("touch_handle_mode:%d",en);

	if(BACK_RESUME == en) {
		core_data->back_resume = true;
		core_data->tp_state_cmd = BACK_RESUME;
		atomic_set(&core_data->touch_handle,BACK_RESUME);
		queue_work(core_data->resume_wq, &core_data->resume_work);
	}else if (BACK_SUSPEND == en) {
		core_data->tp_state_cmd =BACK_SUSPEND;
		core_data->back_resume = false;
		atomic_set(&core_data->touch_handle,BACK_SUSPEND);
		goodix_ts_suspend(core_data);
	}else if (FRONT_SUSPEND == en) {
		ts_info("now to suspend front_tp");
		core_data->tp_state_cmd = FRONT_SUSPEND;
		aod_state_front_tp_to_suspend();
	}
...
#ifdef GOODIX_TS_BACK_PALM
		if ((ts_event->event_type == EVENT_BACK_PALM) && (1 == ts_dev->palm_state)) {
			/* report PALM */
 ...
 * return: 0 ok, <0 failed
 */
int goodix_back_ts_power_on(struct goodix_ts_core *core_data)
{
	struct goodix_ts_board_data *ts_bdata =
		board_data(core_data);
	int r;
...
#endif
#ifdef AOD_BACK_ADJUSTTO_LIGHT
	set_bit(KEY_F15, input_dev->keybit);
#endif
 ...
 */
static int nubia_all_leave(struct goodix_ts_core *core_data,struct input_dev *dev)
{
...
#endif
	ts_info("Back suspend start");

#ifdef NUBIA_GAME_TOUCH_HANDLE
	if(atomic_read(&core_data->touch_handle) == BACK_RESUME) {
		ts_info("Game touch handle mode,no need suspend");
		if (atomic_read(&core_data->panel_state) == TP_ALREADY_SUSPEND) {
			ts_info("TP is suspend,must resume");
			queue_work(core_data->resume_wq, &core_data->resume_work);
		}
		return  -1;
	}
#endif
	if (atomic_read(&core_data->panel_state) == TP_ALREADY_SUSPEND){
		ts_info("Back already suspended");
		return -1;
	}
	/*
...
out:
	/* release all the touch IDs */
	nubia_all_leave(core_data,core_data->input_dev);
	ts_info("Back suspend end");
	return 0;
}
...
static void resume_work_func(struct work_struct *work)
{
	...
	if (!list_empty(&goodix_modules.head)) {
		list_for_each_entry(ext_module, &goodix_modules.head, list) {
			...
			if (r == EVT_CANCEL_RESUME) {
				mutex_unlock(&goodix_modules.mutex);
				ts_info("back Canceled by module:%s", ext_module->name);
				goto out;
			}
		}
	}
	...
	unsigned long event, void *data)
{
	...
	if (BACK_PANEL_EFFECT_MODE == panel_mode)
		core_data->bpe_mode = 1;

	if (fb_event && fb_event->data && core_data) {
		if (event == MSM_DRM_SWITCH_EARLY_EVENT_BLANK) {
			blank = fb_event->data;
			//ts_info("back_mode_power:%4u,%4u",panel_mode,*blank);
			if(BACK_PANEL_EFFECT_MODE == panel_mode) {
				panelDeadState  = 0;
				goodix_ts_suspend(core_data);
				return 0;
			...
			if(READY_BACK_PANEL_MODE == panel_mode && panelDeadState) {
				ts_err("clear panelDeadState when panel_dead only front LCD->OFFON");
				panelDeadState = 0;
			}else if(READY_BACK_PANEL_MODE == panel_mode && core_data->tp_in_aod) {
				ts_err("clear tp_in_aod state");
				core_data->tp_in_aod = 0;
			}

			if (core_data->bpe_mode || core_data->previousEffectState) {
				switch (panel_mode) {
					case READY_BACK_PANEL_MODE:
						panelDeadState  = 0;
						core_data->bpe_mode = 0;
						core_data->previousEffectState = 0;
						goodix_ts_resume(core_data);
						break;
				}
			}

			if (firEffectFlag && core_data->bpe_mode) {
				ts_err("system first start,light effect already open,not resume back_tp.");
				goodix_ts_suspend(core_data);
				firEffectFlag = 0;
				return 0;
			}
			if (core_data->bpe_mode) {
				goodix_ts_suspend(core_data);
			}
			...
			switch (*blank) {
				...
				case MSM_DRM_SLAVE_BLANK_UNBLANK:
					if (panelDeadState) {
						ts_err("PANEL_DEAD,not resume back_tp");
						panelDeadState  = 0;
						return 0;
					} else if (MIRROR_PANEL_OPEN == panel_mode) {
						ts_err("light effect ,not resume back_tp of UNBLANK");
						return 0;
					}else if(core_data->tp_in_aod) {
						ts_err("LCD->OFFON in fast ,not resume back_tp");
						return 0;
					}
					goodix_ts_resume(core_data);
					break;
			...
			blank = fb_event->data;
			switch (*blank) {
				case MSM_DRM_SLAVE_AOD_ON:
					ts_info("Back aod on");
					core_data->tp_in_aod = 1;
					goodix_ts_suspend(core_data);
					break;
				case MSM_DRM_SLAVE_AOD_OFF:
					ts_info("Back aod off");
					if (core_data->bpe_mode) {
						ts_err("light effect ,not resume back_tp of BACK_AOD_OFF");
						core_data->bpe_mode = 0;
						core_data->previousEffectState = 1;
						goodix_ts_suspend(core_data);
						return 0;
					}
					core_data->tp_in_aod = 0;
					if(READY_BACK_PANEL_MODE == panel_mode ||
					    ALREADY_BACK_PANEL_MODE == panel_mode ) {
					goodix_ts_resume(core_data);
						return 0;
					}
					break;
...
static int goodix_ts_probe(struct platform_device *pdev)
{
...
#ifdef NUBIA_GAME_TOUCH_HANDLE
	atomic_set(&core_data->touch_handle,0);
	atomic_set(&core_data->panel_state, TP_ALREADY_RESUME);
#endif
	...
	r = goodix_back_ts_power_on(core_data);
	if (r < 0)
		goto out;
	...
	if (!r)
		...
		back_nopanel = 1;
#ifdef CONFIG_NUBIA_SWITCH_LCD
		goodix_ts_exist_blocking_notify(NUBIA_BACK_LCD_NOT_EXIST);
#endif
		goto out;
	}
