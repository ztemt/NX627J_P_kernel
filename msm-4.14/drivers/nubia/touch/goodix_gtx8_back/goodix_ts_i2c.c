...
#define TS_DT_COMPATIBLE "goodix_back,627"
...
#ifdef GOODIX_TS_BACK_PALM
	if(container_of(&dev, struct goodix_ts_core, ts_dev)->palm_enabled){
		if(1 == dev->palm_state) {
		if(goodix_is_palm(coord_sta) )
		{
			/* mark this event as touch event */
			ts_err("back_coord_sta = %d",coord_sta);
			ts_event->event_type = EVENT_BACK_PALM;
			r=0;
			goto exit_clean_sta;
		}
		}
...
static struct platform_device *goodix_back_pdev;

static void goodix_pdev_release(struct device *dev)
{
	kfree(goodix_back_pdev);
	return;
}

static int goodix_i2c_back_probe(struct i2c_client *client,
	const struct i2c_device_id *dev_id)
{
	...
	ts_info("goodix_i2c_back_probe IN");
	...
	ts_device->name = "GTx5 back TouchDevcie";
	ts_device->dev = &client->dev;
	ts_device->board_data = ts_bdata;
	ts_device->hw_ops = &hw_i2c_ops;

	/* ts core device */
	goodix_back_pdev = kzalloc(sizeof(struct platform_device), GFP_KERNEL);
	if (!goodix_back_pdev)
		return -ENOMEM;

	goodix_back_pdev->name = GOODIX_CORE_DRIVER_NAME;
	goodix_back_pdev->id = 0;
	goodix_back_pdev->num_resources = 0;
	/*
	 * you could find this platform dev in
	 * /sys/devices/platfrom/goodix_ts.0
	 * goodix_pdev->dev.parent = &client->dev;
	 */
	goodix_back_pdev->dev.platform_data = ts_device;
	goodix_back_pdev->dev.release = goodix_pdev_release;

	/* register platform device, then the goodix_ts_core
	 * module will probe the touch deivce. */
	r = platform_device_register(goodix_back_pdev);

	ts_info("goodix_i2c_back_probe back  OUT");

	return r;
}

static int goodix_i2c_back_remove(struct i2c_client *client)
{
	platform_device_unregister(goodix_back_pdev);
	return 0;
}
...
static struct i2c_driver goodix_i2c_back_driver = {
	.driver = {
		.name = TS_DRIVER_NAME,
		.owner = THIS_MODULE,
	...
	.probe = goodix_i2c_back_probe,
	.remove = goodix_i2c_back_remove,
	.id_table = i2c_id_table,
};

static int __init goodix_i2c_back_init(void)
{
	ts_info("back GTx5xx HW layer init");
	return i2c_add_driver(&goodix_i2c_back_driver);
}

static void __exit goodix_i2c_back_exit(void)
{
	i2c_del_driver(&goodix_i2c_back_driver);
	return;
}

module_init(goodix_i2c_back_init);
module_exit(goodix_i2c_back_exit);
