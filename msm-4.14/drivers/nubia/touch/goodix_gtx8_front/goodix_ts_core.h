...
#define NUBIA_FRONT_TOUCH_RESUME_WORKQUEUE
/* macros definition */
#define GOODIX_CORE_DRIVER_NAME	"front_touch"
...
#define CONFIG_FRONT_NUBIA_AOD_MODE_GESTURE_EN
...
/*panel data */
#define GOODIX_TS_FRONT_PALM
#define GOODIX_TS_FRONT_GESTURE
#define GOODIX_FRONT_TS_DYNAMIC_RESUME_SUSPEND

//#define NUBIA_FRONT_SWITCH_TP
#define READY_FRONT_PANEL_MODE        2000

#define READY_BACK_PANEL_MODE         2001
#define ALREADY_BACK_PANEL_MODE	      1001

#define MIRROR_PANEL_OPEN     	      2002
#define MIRROR_PANEL_CLOSE     	      2003

#define DUAL_PANEL_CHANGE_MODE 	      2005
#define FRONT_SUSPEND_MODE	5
#define AOD_FRONT_ADJUSTTO_LIGHT

#define TP_ALREADY_RESUME	1
#define TP_ALREADY_SUSPEND	0
...
/*
 * struct goodix_front_ts_device - ts device data
 * @name: device name
 * @version: reserved
 * @bus_type: i2c or spi
...
struct goodix_ts_hw_ops {
	...
	int (*init)(struct goodix_front_ts_device *dev);
	int (*reset)(struct goodix_front_ts_device *dev);
	int (*read)(struct goodix_front_ts_device *dev, unsigned int addr,
			 unsigned char *data, unsigned int len);
	int (*write)(struct goodix_front_ts_device *dev, unsigned int addr,
			unsigned char *data, unsigned int len);
	int (*read_trans)(struct goodix_front_ts_device *dev, unsigned int addr,
			 unsigned char *data, unsigned int len);
	int (*write_trans)(struct goodix_front_ts_device *dev, unsigned int addr,
			unsigned char *data, unsigned int len);
	int (*send_cmd)(struct goodix_front_ts_device *dev, struct goodix_ts_cmd *cmd);
	int (*send_config)(struct goodix_front_ts_device *dev,
			struct goodix_ts_config *config);
	int (*read_config)(struct goodix_front_ts_device *dev,
						   u8 *config_data, u32 config_len);
	int (*read_version)(struct goodix_front_ts_device *dev,
			struct goodix_ts_version *version);
	int (*event_handler)(struct goodix_front_ts_device *dev,
			struct goodix_ts_event *ts_event);
	int (*check_hw)(struct goodix_front_ts_device *dev);
	int (*suspend)(struct goodix_front_ts_device *dev);
	int (*resume)(struct goodix_front_ts_device *dev);
};
...
struct goodix_ts_core {
	...
	//atomic_t suspended;
#ifdef GOODIX_TS_FRONT_PALM
	int palm_front_enabled;
#endif
	int gameHighFrameRate;
	int diff_cfg_mode;
	int cfg_init;
	/*current panel state*/
	atomic_t panel_state;
	bool cfg_group_parsed;
#ifdef GOODIX_FRONT_TS_DYNAMIC_RESUME_SUSPEND
	bool front_resume;
#endif
	...
	struct work_struct front_resume_work;
	struct workqueue_struct *front_resume_wq;
#endif
#ifdef CONFIG_FB
 ...
 */
static inline struct goodix_front_ts_device *ts_device(
		struct goodix_ts_core *core)
{
	return core->ts_dev;
}
...
/* log macro */
#define ts_info(fmt, arg...)	pr_info("[NUBIA_TS front-INF ][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define	ts_err(fmt, arg...)		pr_err("[NUBIA_TS front-ERR ][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define boot_log(fmt, arg...)	g_info(fmt, ##arg)
#ifdef CONFIG_GOODIX_DEBUG
#define ts_debug(fmt, arg...)	pr_info("[NUBIA_TS front-DBG ][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#else
 ...
 * return: 0 ok, <0 failed
 */
int goodix_front_ts_irq_enable(struct goodix_ts_core *core_data, bool enable);
...
extern int goodix_front_unregister_gesture(void);
#endif
