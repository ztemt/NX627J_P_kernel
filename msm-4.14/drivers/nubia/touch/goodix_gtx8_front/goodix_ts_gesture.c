...
extern void goodix_front_resume_state_set(struct goodix_ts_core *core_data, int v);
		...
		const char *buf, size_t count)
{
	...
	/*This is touch high frame rate ON or OFF when game mode*/
	u8 highFrameRateOn[] = {0x70,0x01,0x8F};
	u8 highFrameRateOff[] = {0x70,0x00,0x90};
	...
	/**
		this switch is choice different cfg when choice edge touch cfg
		GOODIX_CFG_V0: normal or default cfg
		GOODIX_CFG_V1: low   edge touch cfg
		GOODIX_CFG_V2: high edge touch cfg
	*/
	switch (tmp) {
		case GOODIX_CFG_V0:
			ts_debug("cfg change to V0");
			core_data->diff_cfg_mode = GOODIX_CFG_V0;
			r = goodix_front_start_cfg_bin(core_data);
			if (!r)
				ts_info("***start cfg_bin_V0_proc SUCCESS");
			break;
		case GOODIX_CFG_V1:
			ts_debug("cfg change to V1");
			core_data->diff_cfg_mode = GOODIX_CFG_V1;
			r = goodix_front_start_cfg_bin(core_data);
			if (!r)
				ts_info("***start cfg_bin_V1_proc SUCCESS");
			break;
		case GOODIX_CFG_V2:
			ts_debug("cfg change to V2");
			core_data->diff_cfg_mode = GOODIX_CFG_V2;
			r = goodix_front_start_cfg_bin(core_data);
			if (!r)
				ts_info("***start cfg_bin_V2_proc SUCCESS");
			break;
		default:
			break;
	}

	/*game high frame on/off*/
	if(1 == tmp) {
		ts_debug("tp game high frame rate on");
		core_data->gameHighFrameRate = 1;
		ts_dev->hw_ops->write_trans(ts_dev,0x6F68,&highFrameRateOn[0],3);
	}else if (0 == tmp) {
		ts_debug("tp game high frame rate off");
		core_data->gameHighFrameRate = 0;
		ts_dev->hw_ops->write_trans(ts_dev,0x6F68,&highFrameRateOff[0],3);
	}
				...
				char *buf)
{
	...
	if ((buf == NULL) || (gsx_gesture->gesture_data == NULL)) {
		ts_info("Parameter error!");
		return -EPERM;
	}
	read_lock(&gsx_gesture->rwlock);
...
const struct goodix_ext_attribute front_gesture_attrs[] = {
...
int goodix_front_unregister_gesture(void){
	int ret = 0;
	if (!atomic_read(&gsx_gesture->registered)) {
		ts_debug("Gesture module has aready unregistered");
		return -1;
	}
	ts_debug("Start front unregistered gesture module");
	ret = goodix_front_register_ext_module(&gsx_gesture->module);
	if (!ret) {
		...
		struct goodix_ext_module *module)
{
	...
	ret = kobject_init_and_add(&module->kobj, goodix_front_get_default_ktype(),
	...
	for (i = 0; i < sizeof(front_gesture_attrs)/sizeof(front_gesture_attrs[0]); i++) {
		if (sysfs_create_file(&module->kobj,
				&front_gesture_attrs[i].attr)) {
	...
	struct goodix_ext_module *module)
{
	...
	/*ts_debug("gsx_gesture_ist, core_data-suspend=%d",
			atomic_read(&core_data->panel_state));*/
	...
	ts_debug("front data[0-4]0x%x, 0x%x, 0x%x, 0x%x", temp_data[0], temp_data[1],
	...
	if (QUERYBIT(gsx_gesture->gesture_type, temp_data[2])) {
		/* do resume routine */
		ts_info("front Gesture match success, resume IC");

#ifdef AOD_FRONT_ADJUSTTO_LIGHT
	if (aod_light == temp_data[2]) {
		ts_info("now need to resume front panel");
		goodix_front_resume_state_set(core_data, 1);
		input_report_key(core_data->input_dev, KEY_F14, 1);
		input_sync(core_data->input_dev);
		input_report_key(core_data->input_dev, KEY_F14, 0);
		input_sync(core_data->input_dev);
...
static int __init goodix_gsx_gesture_init(void)
{
	...
	int result;
	ts_info("front gesture module init");
	...
	gsx_gesture->module.name = "Goodix_front_gsx_gesture";
	gsx_gesture->module.priv_data = gsx_gesture;
	gsx_gesture->kobj_initialized = 0;
	atomic_set(&gsx_gesture->registered, 0);
	rwlock_init(&gsx_gesture->rwlock);
	result = goodix_front_register_ext_module(&(gsx_gesture->module));
	if (result == 0)
		atomic_set(&gsx_gesture->registered, 1);
