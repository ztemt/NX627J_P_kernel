...
#define GOOIDX_INPUT_PHYS	"front_touch/input2"
#define PINCTRL_STATE_ACTIVE    "pmx_ts_front_active"
#define PINCTRL_STATE_SUSPEND   "pmx_ts_front_suspend"

extern int goodix_front_start_cfg_bin(struct goodix_ts_core *ts_core);
#ifdef GOODIX_TS_FRONT_GESTURE
/*This flag was used to power off in suspend when gesture function is disable*/
int f_gesture_flag = 0;
#endif
...
struct goodix_module goodix_front_module;
...
static void  __do_register_ext_module(struct work_struct *work)
{
	...
	struct list_head *insert_point = &goodix_front_module.head;

	ts_info("__do_register_ext_module IN, goodix_front_module.core_exit:%d", goodix_front_module.core_exit);

	/* waitting for core layer */
	if (!wait_for_completion_timeout(&goodix_front_module.core_comp, 25 * HZ)) {
		ts_err("Module [%s] timeout", module->name);
		return;
	}

	/* driver probe failed */
	if (goodix_front_module.core_exit) {
	...
	mutex_lock(&goodix_front_module.mutex);
	if (!list_empty(&goodix_front_module.head)) {
		list_for_each_entry(ext_module, &goodix_front_module.head, list) {
			if (ext_module == module) {
				ts_info("Module [%s] already exists",
						module->name);
				mutex_unlock(&goodix_front_module.mutex);
				return;
			}
		}
		...
		list_for_each_entry(ext_module, &goodix_front_module.head, list) {
		 ...
		 to goodix_front_module->head */
	}

	if (module->funcs && module->funcs->init) {
		if (module->funcs->init(goodix_front_module.core_data,
					module) < 0) {
...
/**
 * goodix_front_register_ext_module - interface for external module
 * to register into touch core modules structure
 *
 * @module: pointer to external module to be register
 * return: 0 ok, <0 failed
 */
int goodix_front_register_ext_module(struct goodix_ext_module *module)
{
	if (!module)
		return -EINVAL;

	if (!goodix_front_module.initilized) {
		goodix_front_module.initilized = true;
		goodix_front_module.core_exit = true;
		INIT_LIST_HEAD(&goodix_front_module.head);
		mutex_init(&goodix_front_module.mutex);
		init_completion(&goodix_front_module.core_comp);
	}
...
*/
	ts_info("goodix_front_register_ext_module IN");

	INIT_WORK(&module->work, __do_register_ext_module);
	schedule_work(&module->work);

	ts_info("goodix_front_register_ext_module OUT");


	return 0;
}
EXPORT_SYMBOL_GPL(goodix_front_register_ext_module);

/**
 * goodix_front_unregister_ext_module - interface for external module
 * to unregister external modules
 *
 * @module: pointer to external module
 * return: 0 ok, <0 failed
 */
int goodix_front_unregister_ext_module(struct goodix_ext_module *module)
{
	struct goodix_ext_module *ext_module;
	bool found = false;
	...
	if (!goodix_front_module.core_data)
		return -ENODEV;
	...
	if (module->funcs && module->funcs->exit)
		module->funcs->exit(goodix_front_module.core_data, module);
	goodix_front_module.count--;

	ts_info("Moudle [%s] unregistered",
			module->name ? module->name : " ");
	//if(!strncmp(module->name, "goodix-fwu", strlen("goodix-fwu"))){
		//goodix_ts_suspend(goodix_front_module.core_data);
	//}
	return 0;
}
EXPORT_SYMBOL_GPL(goodix_front_unregister_ext_module);
...
struct kobj_type *goodix_front_get_default_ktype(void)
{
	return &goodix_ext_ktype;
}
EXPORT_SYMBOL_GPL(goodix_front_get_default_ktype);

struct kobject *goodix_front_get_default_kobj(void)
{
	struct kobject *kobj = NULL;

	if (goodix_front_module.core_data &&
			goodix_front_module.core_data->pdev)
		kobj = &goodix_front_module.core_data->pdev->dev.kobj;
	return kobj;
}
EXPORT_SYMBOL_GPL(goodix_front_get_default_kobj);
...
void goodix_front_msg_printf(const char *fmt, ...)
{
	va_list args;
	int r;

	if (goodix_front_dbg.pos < goodix_front_dbg.buf.size) {
		va_start(args, fmt);
		r = vscnprintf(goodix_front_dbg.buf.data + goodix_front_dbg.pos,
			 goodix_front_dbg.buf.size - 1, fmt, args);
		goodix_front_dbg.pos += r;
		va_end(args);
	}
}
EXPORT_SYMBOL_GPL(goodix_front_msg_printf);

static int goodix_debugfs_init(void)
{
	struct dentry *r_b;
	goodix_front_dbg.buf.size = PAGE_SIZE;
	goodix_front_dbg.pos = 0;
	goodix_front_dbg.buf.data = kzalloc(goodix_front_dbg.buf.size, GFP_KERNEL);
	if (goodix_front_dbg.buf.data == NULL) {
		pr_err("Debugfs init failed\n");
		goto exit;
	}
	r_b = debugfs_create_blob("goodix_ts", 0644, NULL, &goodix_front_dbg.buf);
	if (!r_b) {
		pr_err("Debugfs create failed\n");
		return -ENOENT;
	}
	goodix_front_dbg.dentry = r_b;
...
{
	debugfs_remove(goodix_front_dbg.dentry);
	goodix_front_dbg.dentry = NULL;
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&goodix_front_module.mutex);
	if (!list_empty(&goodix_front_module.head)) {
		list_for_each_entry(module, &goodix_front_module.head, list) {
		...
		size_t count)
{
	...
	goodix_front_ts_irq_enable(core_data, en);
	return count;
}

#ifdef GOODIX_TS_FRONT_PALM
int f_palm_enabled;
static ssize_t goodix_ts_front_palm_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	struct goodix_ts_core *core_data =
	...
	r = snprintf(buf, PAGE_SIZE, "%d\n",
			core_data->f_palm_enabled);
	if (r < 0)
		return -EINVAL;

	return r;
}
static ssize_t goodix_ts_front_palm_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf,
		size_t count)
{
	...
	core_data->f_palm_enabled = en;
	return count;
}
#endif

void goodix_front_resume_state_set(struct goodix_ts_core *core_data, int v)
{
	if(v){
		queue_work(core_data->front_resume_wq, &core_data->front_resume_work);
	}else{
...
#ifdef GOODIX_TS_FRONT_PALM
static DEVICE_ATTR(palm, S_IRUGO | S_IWUSR | S_IWGRP, goodix_ts_front_palm_show, goodix_ts_front_palm_store);
#endif
...
int goodix_front_ts_sysfs_init(struct goodix_ts_core *core_data)
{
 ...
 */
int goodix_front_ts_register_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&ts_notifier_list, nb);
}
EXPORT_SYMBOL(goodix_front_ts_register_notifier);
 ...
 *	see enum ts_notify_event in goodix_ts_core.h
 */
int goodix_front_ts_unregister_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&ts_notifier_list, nb);
}
EXPORT_SYMBOL(goodix_front_ts_unregister_notifier);

/**
 * fb_notifier_call_chain - notify clients of fb_events
 *	see enum ts_notify_event in goodix_ts_core.h
 */
int goodix_front_ts_blocking_notify(enum ts_notify_event evt, void *v)
{
	return blocking_notifier_call_chain(&ts_notifier_list,
			(unsigned long)evt, v);
}
EXPORT_SYMBOL_GPL(goodix_front_ts_blocking_notify);

#ifdef GOODIX_TS_FRONT_PALM
static int goodix_ts_front_palm_report(struct input_dev *dev)
{
	//ts_info(" %s",__func__);
...
/**
 * goodix_ts_front_input_report - report touch event to input subsystem
 *
 * @dev: input device pointer
 * @touch_data: touch data pointer
 * return: 0 ok, <0 failed
 */
static int goodix_ts_front_input_report(struct input_dev *dev,
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	if (likely(r >= 0)) {
		if (ts_event->event_type == EVENT_TOUCH) {
			/* report touch */
			goodix_ts_front_input_report(core_data->input_dev,
					&ts_event->event_data.touch_data);
		}
#ifdef GOODIX_TS_FRONT_PALM
		if (ts_event->event_type == EVENT_FRONT_PALM) {
			/* report PALM */
			goodix_ts_front_palm_report(core_data->input_dev);
		}
#endif
	}
 ...
 * return: 0 ok, <0 failed
 */
int goodix_front_ts_irq_setup(struct goodix_ts_core *core_data)
{
...
/**
 * goodix_front_ts_irq_enable - Enable/Disable a irq
 * @core_data: pointer to touch core data
 * enable: enable or disable irq
 * return: 0 ok, <0 failed
 */
int goodix_front_ts_irq_enable(struct goodix_ts_core *core_data,
			bool enable)
{
	if (enable) {
...
EXPORT_SYMBOL(goodix_front_ts_irq_enable);
/**
...
/*add power on 1.8V */
int goodix_ts_gpio_power_request(struct goodix_ts_core *core_data, bool reg)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);
	int r;
	unsigned char buf[16];
	ts_info("Device power on 1V8");
	if(reg) {
	   snprintf(buf,PAGE_SIZE,"goodix_gpio_%u\n",ts_bdata->power_gpio);
	   r = gpio_request(ts_bdata->power_gpio,buf);
	   if(r) {
	      ts_err("Failed to get gpio %d",
			ts_bdata->power_gpio);	
	      return r;
	   }	
	   r = gpio_direction_output(ts_bdata->power_gpio,1);
	   if(r) {
	      ts_err("Failed to set gpio %d direction",
		      ts_bdata->power_gpio);
	      return r;
	   }
	} else {
	   gpio_free(ts_bdata->power_gpio);
	}
	return 0;
}
int goodix_ts_front_gpio_power_on_off(struct goodix_ts_core *core_data, bool on)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);
	if(on) {
	   if(ts_bdata->power_gpio >= 0)
		gpio_set_value(ts_bdata->power_gpio, 1);
	} else {
	   if(ts_bdata->power_gpio >= 0)
		gpio_set_value(ts_bdata->power_gpio, 0);
	}
	return 0;
}
/**
 * goodix_front_ts_power_on - Turn on power to the touch device
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
int goodix_front_ts_power_on(struct goodix_ts_core *core_data)
{
	struct goodix_ts_board_data *ts_bdata =
			board_data(core_data);
	int r;
...
/**
 * goodix_front_ts_power_off - Turn off power to the touch device
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
int goodix_front_ts_power_off(struct goodix_ts_core *core_data)
{
	struct goodix_ts_board_data *ts_bdata =
			board_data(core_data);
	int r;
 ...
 *	reset_gpio and irq_gpio number are obtained from goodix_front_ts_device
 *  which created in hardware layer driver. e.g.goodix_xx_i2c.c
 *	A goodix_front_ts_device should set those two fileds to right value
...
#ifdef AOD_FRONT_ADJUSTTO_LIGHT
	set_bit(KEY_F14, input_dev->keybit);
#endif
...
/**
 * goodix_front_ts_input_dev_config - Requset and config a input device
 ...
 * return: 0 ok, <0 failed
 */
int goodix_front_ts_input_dev_config(struct goodix_ts_core *core_data)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);
...
/**
 * goodix_front_ts_hw_init - Hardware initilize
 *  poweron - hardware reset - sendconfig
 * @core_data: pointer to touch core data
 * return: 0 intilize ok, <0 failed
 */
int goodix_front_ts_hw_init(struct goodix_ts_core *core_data)
{
...
static void goodix_ts_esd_work(struct work_struct *work)
{
	...
	if (r < 0) {
		goodix_front_ts_power_off(core);
		goodix_front_ts_power_on(core);
		if (hw_ops->reset)
...
/**
 * goodix_front_ts_esd_init - initialize esd protection
 */
int goodix_front_ts_esd_init(struct goodix_ts_core *core)
{
	struct goodix_ts_esd *ts_esd = &core->ts_esd;
	struct goodix_front_ts_device *dev = core->ts_dev;
	u8 data = GOODIX_ESD_TICK_WRITE_DATA;
	int r;
	...
	goodix_front_ts_register_notifier(&ts_esd->esd_notifier);
 ...
 */
static int nubia_front_all_leave(struct goodix_ts_core *core_data,struct input_dev *dev)
{
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	...
	struct goodix_front_ts_device *ts_dev = core_data->ts_dev;
	int r;
#ifdef NUBIA_FRONT_TOUCH_RESUME_WORKQUEUE
	flush_work(&core_data->front_resume_work);
#endif
	ts_info("Front suspend start");

	if (atomic_read(&core_data->panel_state) == TP_ALREADY_SUSPEND){
		ts_info("Front already suspended");
		return -1;
	}
	/*
	 ...
	 */
	goodix_front_ts_blocking_notify(NOTIFY_SUSPEND, NULL);
	/* inform external module */
	mutex_lock(&goodix_front_module.mutex);
	if (!list_empty(&goodix_front_module.head)) {
		list_for_each_entry(ext_module, &goodix_front_module.head, list) {
	...
	/* disable irq */
	goodix_front_ts_irq_enable(core_data, false);

	/* let touch ic work in sleep mode */
	if (ts_dev && ts_dev->hw_ops->suspend)
		ts_dev->hw_ops->suspend(ts_dev);
	atomic_set(&core_data->panel_state, TP_ALREADY_SUSPEND);
...
out:
	nubia_front_all_leave(core_data,core_data->input_dev);
	ts_info("Front suspend end");
	return 0;
}
#ifndef NUBIA_FRONT_TOUCH_RESUME_WORKQUEUE
...
{
	struct goodix_ext_module *ext_module;
	struct goodix_front_ts_device *ts_dev =
				core_data->ts_dev;
	int r;

	ts_info("Front Resume start");
...
#endif
	/* nubia add for power on start */
	atomic_set(&core_data->panel_state, 0);
	/* resume device */
	...
	mutex_unlock(&goodix_front_module.mutex);
	goodix_front_ts_irq_enable(core_data, true);
out:
	goodix_front_ts_blocking_notify(NOTIFY_RESUME, NULL);
	ts_debug("Resume end");
	return 0;
}
#else
static void resume_front_work_func(struct work_struct *work)
{
	struct goodix_ts_core *core_data =
	...
	struct goodix_ext_module *ext_module;
	struct goodix_front_ts_device *ts_dev = core_data->ts_dev;
	u8 highFrameRateOn[] = {0x70,0x01,0x8F};
	int r;
	ts_info("Front resume start\n");

#ifdef CONFIG_FRONT_NUBIA_AOD_MODE_GESTURE_EN
	if (atomic_read(&core_data->panel_state) == TP_ALREADY_RESUME){
		ts_info("Front already resumed");
		goto out;
	}
#endif
	...
	if (!list_empty(&goodix_front_module.head)) {
		list_for_each_entry(ext_module, &goodix_front_module.head, list) {
			...
			if (r == EVT_CANCEL_RESUME) {
				mutex_unlock(&goodix_front_module.mutex);
				ts_info("front Canceled by module:%s", ext_module->name);
				goto out;
			}
		}
	}
	...
	goodix_front_ts_irq_enable(core_data, true);
#ifdef CONFIG_FRONT_NUBIA_AOD_MODE_GESTURE_EN
	atomic_set(&core_data->panel_state, TP_ALREADY_RESUME);
#endif
out:
	/*
	 ...
	 */
	 if(core_data->gameHighFrameRate) {
		ts_dev->hw_ops->write_trans(ts_dev,0x6F68,&highFrameRateOn[0],3);
	 }
	goodix_front_ts_blocking_notify(NOTIFY_RESUME, NULL);
	ts_debug("Front resume end");
}
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	queue_work(core_data->front_resume_wq,
		&core_data->front_resume_work);
	return 0;
}
#endif
...
/**
 * goodix_front_ts_fb_notifier_callback - Framebuffer notifier callback
 * Called by kernel during framebuffer blanck/unblank phrase
 */
 
void aod_state_front_tp_to_suspend()
 {
	ts_info("front_tp ready suspend");
	if (goodix_front_module.core_data->f_aod_mode)
		goodix_front_resume_state_set(goodix_front_module.core_data, 0);
	return;
 }
 
int goodix_front_ts_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
	...
	if (fb_event && fb_event->data && core_data) {
		if (event == MSM_DRM_SWITCH_EARLY_EVENT_BLANK) {
			blank = fb_event->data;
			//ts_debug("front_mode_power:%4u,%4u",panel_mode,*blank);
			switch (*blank) {
			...
			switch (*blank) {
				case MSM_DRM_MAJOR_AOD_ON:
					ts_debug("Front aod on");
					core_data->f_aod_mode = 1;
					goodix_ts_suspend(core_data);
					break;
				case MSM_DRM_MAJOR_AOD_OFF:
					ts_debug("Front aod off");
					core_data->f_aod_mode = 0;
					goodix_ts_resume(core_data);
					break;
				default:
					break;
...
/**
 * goodix_front_generic_noti_callback - generic notifier callback
 *  for goodix touch notification event.
 */
int goodix_front_generic_noti_callback(struct notifier_block *self,
		unsigned long action, void *data)
{
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	struct goodix_front_ts_device *ts_device;
	int r;
	u8 read_val = 0;
	...
	r = goodix_ts_gpio_power_request(core_data,true);
	if (r < 0)
		goto out;
	r = goodix_front_ts_power_on(core_data);
	if (r < 0)
		goto out;
	r = goodix_ts_front_gpio_power_on_off(core_data,true);
	if (r)
		goto out;
	...
	/*unified protocl
	  *start a thread to parse cfg_bin and init IC
	    core_data->diff_cfg_mode = CFG_V0 : default is 0
	  */
	core_data->diff_cfg_mode = GOODIX_CFG_V0;
	core_data->cfg_init = 0;
	r = goodix_front_start_cfg_bin(core_data);
	if (!r) {
...
#ifdef NUBIA_FRONT_TOUCH_RESUME_WORKQUEUE
	core_data->front_resume_wq =
			create_singlethread_workqueue("goodix_front_resume_workqueue");
	INIT_WORK(&core_data->front_resume_work, resume_front_work_func);
#endif
...
#ifdef NUBIA_FRONT_TOUCH_RESUME_WORKQUEUE
	cancel_work_sync(&core_data->front_resume_work);
	flush_workqueue(core_data->front_resume_wq);
	destroy_workqueue(core_data->front_resume_wq);
#endif
	goodix_front_ts_power_off(core_data);
	goodix_debugfs_exit();
