...
#define DELAY_MASTERERASE_MS    (1000 * 1000) 
...
static void get_fan_read(void){
	...
	if((get_fan_power_on_state() == true) ){
		...
		{
		    printk(KERN_ERR "%s:fan_count= %d,fan_speed= %d\n",fan_count,fan_speed);
...
static void fan_set_pwm_by_level(unsigned int level)
{
	...
	if(level < FAN_LEVEL_0 || level > FAN_LEVEL_MAX)
		return;
    if(get_fan_power_on_state() == false && level == 0)
		return;
...
static int fan_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	ret = regulator_set_voltage(fan->pwr_reg, 3000000, 3000000);
	if (ret) {
	        dev_err(fan->dev->parent,
    ...
    nubia_fan = fan;
    gpio11_test =of_get_named_gpio(np,"reset-gpios",0);
